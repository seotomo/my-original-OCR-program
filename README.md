...
## 目的
既存のOCRソフトは数多くあるが、日本語の文字認識は非常に難しいと言われている。理由として①ひらがな・カタカナに似た文字が多い、②「け」や「は」など独立した複数の画から構成される文字が多い、などが挙げられる。特に手書き文字の認識では精度が低いと言われる。AIを用いたOCRでは文書から文字を抽出するために画像処理、文字の分類に深層学習を用いており、私が大学院の研究で培ったこれらの技術がOCRの作成に生かせるのではないかと考えた。そこで、自身のスキルがどこまで通用するかという意味でも手書き文字に対応したOCRを自作し、既存のものと比べどこまで精度が出せるかを試してみた。

## 概要
### 機能

文書画像ファイルを読み込み、文字認識を行う。手動での設定項目として読み込んだ画像ファイルの文書形式がある。  
文字認識後には認識された文字が**テキストファイル**として出力される。
<br>
<br>
### 対応文書
1.	スキャンされた手書き文字
...
<br> 
### 対応文字
ひらがな（濁音含む）、カタカナ（濁音除く）、漢字（JIS第１水準）、数字、アルファベット（大文字小文字）、特殊文字の一部  
計3157文字に対応している。
<br>  
<br>
## OCRプログラム
...
<br>
<img src="/images/ocr_images/抽出した文字(例).jpg" width = "700">  
　　　　　　　　　　　　　　　　　　　__抽出した文字__  
