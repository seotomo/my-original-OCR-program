...
## 目的
既存のOCRソフトは数多くあるが、日本語の文字認識は非常に難しいと言われている。理由として①ひらがな・カタカナに似た文字が多い、②「け」や「は」など独立した複数の画から構成される文字が多い、などが挙げられる。特に手書き文字の認識では精度が低いと言われる。AIを用いたOCRでは文書から文字を抽出するために画像処理、文字の分類に深層学習を用いており、私が持つこれらの技術がOCRの作成に生かせるのではないかと考えた。そこで、自身のスキルがどこまで通用するかという意味でも手書き文字に対応したOCRを自作し、既存のものと比べどこまで精度が出せるかを試してみた。
