...
pyocr+tessaractの結果では手書き文字で30%、印刷文字でほぼ100%の精度であった。規則正しく並んだ文字に対しては既存のOCRツールのほうが自作OCRより精度が高かったが、手書き文字では大きく精度が落ちていた。罫線（直線）の除去等の処理を追加することで改善が見られそうだが、やはり手書き文字には手書き文字に特化したAIによる学習が有効だと感じた。
<br>
<br>
