...
#files: ETL6データのファイル名
files = ["ETL6C_"+str(f'{i:02}') for i in range(1,13)]
RECORD_SIZE = 2052
MARGIN = 10
...
for file in files:
    ...
    with open(file, 'rb') as f:
        while True:
            ...
            #読み込んだ画像の前処理
            img = np.array(img)
            _, img = cv2.threshold(img,3,255,cv2.THRESH_OTSU)
            ...
            os.makedirs(f"ETL6/{dirname}",exist_ok=True)
    
            imagefile = f"ETL6/{dirname}/{file}_{i:0>6}.png"
